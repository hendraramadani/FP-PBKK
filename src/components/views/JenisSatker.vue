...
export default {
  ...
  methods: {
    add () {
      axios.post('http://10.199.14.46:8010/api/jenissatker/').then(res => {
        console.log('adding data in new row')
      })
    },
    update (instance, cell, columns, row, value) {
      axios.get('http://10.199.14.46:8010/api/jenissatker/').then(res => {
        var index = Object.values(res.data[row])
        index[columns] = value
        console.log(index)
        axios.put('http://10.199.14.46:8010/api/jenissatker/' + index[0], {
          id: index[0],
          nama: index[1],
    ...
    delete (instance, row) {
      axios.get('http://10.199.14.46:8010/api/jenissatker/').then(res => {
        var index = Object.values(res.data[row])
        axios.delete('http://10.199.14.46:8010/api/jenissatker/' + index[0])
  ...
  computed: {
    jexcelOptions () {
      return {
        allowToolbar: true,
        url: 'http://10.199.14.46:8010/api/jenissatker/',
